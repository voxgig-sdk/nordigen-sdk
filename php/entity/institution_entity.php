...
// Nordigen SDK Institution entity
...
class InstitutionEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new InstitutionEntity($this->_client, $opts);
    }

    /**
     * @param Institution|array $args Institution data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = NordigenHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Institution|array The current Institution data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Institution fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = NordigenHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Institution fields).
     */
    ...
    /**
     * Load a single Institution.
     *
     * @param InstitutionLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed InstitutionLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Institution|array The loaded Institution as an assoc-array at the
     *   SDK boundary; throws NordigenError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = NordigenHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Institution items matching the given filter.
     *
     * @param InstitutionListMatch|array|null $reqmatch Match filter (any subset
     *   of Institution fields) as an assoc-array; InstitutionListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Institution[]|array A list of Institution items as assoc-arrays at
     *   the SDK boundary; throws NordigenError on failure (item-5 convention).
     */
