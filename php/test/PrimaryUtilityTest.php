...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        NordigenSDK $client,
        NordigenUtility $utility
    ): NordigenContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new NordigenContext($ctxmap, null);
    ...
    private static function fixctx(NordigenContext $ctx, NordigenSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?NordigenError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new NordigenError($code, $msg);
    }
    ...
    {
        $client = NordigenSDK::test(null, null);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new NordigenBaseFeature();
    ...
    {
        $hook_client = NordigenSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends NordigenBaseFeature {
            public $hook_fn = null;
            public function TestHook(NordigenContext $ctx): void
            {
    ...
    {
        $init_client = NordigenSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends NordigenBaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new NordigenSDK([
            'base' => 'http://localhost:8080',
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new NordigenSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = NordigenSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = NordigenSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof NordigenError) ? $result->err->msg : (string)$result->err;
