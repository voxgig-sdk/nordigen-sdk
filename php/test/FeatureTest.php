...
class FtCtrl extends NordigenControl
{
...
class FtClient
{
    ...
    private NordigenUtility $utility;

    public function __construct(NordigenUtility $utility, array $options)
    {
    ...
    public function get_utility(): NordigenUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NordigenContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NordigenContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NordigenContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NordigenConfig::make_feature($name);
    ...
    private static function buildUrl(NordigenSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NordigenError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NordigenResponse($fetched);
        }
    ...
    private function fail(NordigenContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NordigenContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
