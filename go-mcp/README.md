...
[MCP](https://modelcontextprotocol.io) server exposing the Nordigen SDK as
two agent tools — `nordigen_list` and `nordigen_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nordigen-mcp)
make build

# 2. Provide credentials via the environment
export NORDIGEN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user nordigen \
  -- /absolute/path/to/nordigen-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./nordigen-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// nordigen_list: first page of records
{ "entity": "institution" }
{ "entity": "institution", "query": { } }

// nordigen_load: one record by id
{ "entity": "institution", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nordigen-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user nordigen \
     -- "$PWD"/dist/*/nordigen-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `nordigen_list` and `nordigen_load` tools now appear
   in new sessions. Ask the agent to *"list institution using nordigen"*
   and it calls `nordigen_list` with `{"entity":"institution"}`.
...
```sh
export NORDIGEN_APIKEY=sk_live_xxx            # API key
export NORDIGEN_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `nordigen_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `nordigen_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NORDIGEN_APIKEY` | API key sent with every request. |
| `NORDIGEN_BASE` | Optional override of the API base URL. |
...
```sh
./nordigen-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"nordigen_load","arguments":{"entity":"institution","query":{"id":1}}}}'
```
