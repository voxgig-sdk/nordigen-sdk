...
local InstitutionEntity = {}
InstitutionEntity.__index = InstitutionEntity


function InstitutionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InstitutionEntity)
...
function InstitutionEntity:get_name()
  return self._name
end


function InstitutionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InstitutionEntity.new(self._client, opts)
end
...
function InstitutionEntity:mark_deleted()
  self._deleted = true
end


function InstitutionEntity:deleted()
  return true == self._deleted
end


function InstitutionEntity:data_set(args)
  if args ~= nil then
...
function InstitutionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InstitutionEntity:match_set(args)
  if args ~= nil then
...
function InstitutionEntity:match_get()
...
function InstitutionEntity:stream(action, args, callopts)
...
---@param reqmatch InstitutionLoadMatch
---@param ctrl? table
---@return Institution
---@return string? err
function InstitutionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch InstitutionListMatch
---@param ctrl? table
---@return Institution[]
---@return string? err
function InstitutionEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function InstitutionEntity:_run_op(ctx, post_done)
  local utility = self._utility
