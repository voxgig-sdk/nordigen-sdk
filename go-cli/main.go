// Command Nordigen-cli is a CLI + REPL for the Nordigen SDK,
...
const prompt = "nordigen"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "institution"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      NORDIGEN_APIKEY (api key), NORDIGEN_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: NORDIGEN_APIKEY carries the API key and
	// NORDIGEN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NORDIGEN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NORDIGEN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNordigenSDK(opts)
