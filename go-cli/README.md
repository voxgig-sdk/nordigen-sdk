...
boru-driven command-line client **and** interactive REPL for the Nordigen
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nordigen-cli)
make build
...
./nordigen-cli list institution
./nordigen-cli load 1 institution            # {id:1} shorthand
./nordigen-cli load '{id:1}' institution       # explicit match map

# 5. Override the API base URL for a single call
NORDIGEN_BASE=https://api.example.com ./nordigen-cli list institution

# 6. No arguments -> interactive REPL
./nordigen-cli
nordigen> list institution
nordigen> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nordigen-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/nordigen-cli list institution
   ```
...
```sh
./nordigen-cli load 1 institution          # scalar shorthand for {id:1}
./nordigen-cli load '{id:1}' institution     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `nordigen>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `institution`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/nordigen-cli`. |
...
(`./nordigen-cli <expr>`) and interactively (the REPL), and expressions compose the
