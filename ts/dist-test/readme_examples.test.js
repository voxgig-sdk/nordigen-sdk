...
const TEST_SEED = { "entity": { "institution": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+NordigenSDK\b/.test(code) ||
        /\bNordigenSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+NordigenSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/NordigenSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
