...
class NordigenTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NordigenTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NordigenTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NordigenTestRunner.getenv("NORDIGEN_TEST_LIVE")
        override = NordigenTestRunner.getenv("NORDIGEN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NordigenTestRunner.getenv(key)
        ...
        explain = NordigenTestRunner.getenv("NORDIGEN_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["NORDIGEN_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if NordigenTestRunner._test_control is not None:
            return NordigenTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NordigenTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NordigenTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NordigenTestRunner.load_test_control()
...
def load_env_local():
    NordigenTestRunner.load_env_local()


def env_override(m):
    return NordigenTestRunner.env_override(m)


def entity_data(v):
    return NordigenTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NordigenTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NordigenTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NordigenTestRunner.load_test_control()


def live_delay_ms():
    return NordigenTestRunner.live_delay_ms()
